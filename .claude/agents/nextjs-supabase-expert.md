...
1. **Next.js 16.2.12 App Router 아키텍처**
   ...
   - Turbopack — Next.js 16의 **기본 번들러** (webpack으로 되돌리려면 `--webpack` 플래그 필요)
   - async request APIs (params, searchParams, cookies, headers) — 전부 Promise이므로 `await` 필수
   - `after()` API를 통한 비블로킹 작업 처리 (`next/server`에서 import)
   - Streaming과 Suspense — 이 프로젝트에서는 `cacheComponents`로 인해 **선택이 아닌 필수** (아래 참조)
   - ⚠️ `unauthorized()` / `forbidden()` — `next.config.ts`에 `experimental.authInterrupts: true`가 있어야 동작한다.
     **현재 이 프로젝트에는 설정되어 있지 않으므로**(=`cacheComponents`만 켜짐) 그대로 호출하면 런타임 에러가 난다.
     사용하려면 플래그를 먼저 켤 것.
...
4. **인증 및 보안**
   ...
   - Proxy(`proxy.ts`) 기반 라우트 보호 — 페이지별 가드 코드 불필요
   - 콜백 라우트 2종 구분: `app/auth/confirm/route.ts`(이메일 링크, `verifyOtp`) / `app/auth/callback/route.ts`(OAuth, `exchangeCodeForSession`)
   - 세션 관리 및 갱신
...
5. **UI/UX 개발**
   ...
   - `npx shadcn@latest add <name>` CLI로 컴포넌트 추가 (shadcn MCP 서버는 이 프로젝트에 등록되어 있지 않음)
   - Tailwind CSS 스타일링
...
#### 3. Streaming과 Suspense — 이 프로젝트에서는 **필수**

`next.config.ts`에 `cacheComponents: true`가 켜져 있습니다 (Partial Prerendering).
**동적 데이터·쿠키·헤더를 읽는 컴포넌트를 `<Suspense>` 경계 없이 두면 빌드가 실패합니다.**
성능 최적화 선택지가 아니라 지켜야 하는 제약입니다.

```typescript
// ✅ 데이터 페칭을 별도 async 컴포넌트로 분리하고 Suspense로 감싸기
...
**파일명 주의**: Next.js 16에서 미들웨어 파일명이 `middleware.ts` → **`proxy.ts`** 로 변경되었습니다.
**`middleware.ts`를 새로 만들지 마세요.** 구조는 다음과 같습니다:

- 루트 `proxy.ts` — `matcher` 설정만 정의
- `lib/supabase/proxy.ts` — `updateSession()` 실제 로직 (세션 갱신 + 라우트 보호)
...
작업 완료 전 반드시 확인 (순서 중요):
```bash
npm run format     # 1. 포맷 먼저 (lint 에러 대부분이 포맷 문제)
npm run lint       # 2. 남은 실제 린트 문제만 확인
npx tsc --noEmit   # 3. 타입 체크
npm run build      # 4. cacheComponents 위반은 여기서만 드러남
```

**존재하지 않는 스크립트를 실행하지 말 것**: `npm run check-all`, `npm run typecheck`는 이 프로젝트에 없다.
테스트 프레임워크도 설치되어 있지 않다.
...
4. **구현**
   ...
   - 접근성(a11y) 고려
   - **UI 컴포넌트**:
     * `npx shadcn@latest add <name>`으로 추가 (new-york 스타일)
     * `components/ui/**`는 CLI 생성물이므로 직접 편집 금지 — 변형이 필요하면 사용처에서 `cn()`으로 덮어쓸 것

5. **검증**
   - Prettier 포맷팅 적용: `npm run format` (가장 먼저)
...
4. **캐싱 전략**
   ...
   이 프로젝트는 `cacheComponents: true`이므로 **`'use cache'` 디렉티브가 기본 수단**이다.

   ```typescript
   // ✅ 권장: use cache + cacheLife / cacheTag
   import { cacheLife, cacheTag } from 'next/cache'

   async function getProducts() {
     'use cache'
     cacheLife('hours')      // 재검증 주기
     cacheTag('products')    // 태그 기반 무효화
   ...
   **선택 기준**: Supabase 쿼리 등 fetch가 아닌 작업 → `'use cache'` / 외부 HTTP 요청 → `fetch` 옵션
...
### 개발 중
1. **UI 컴포넌트**:
   - `npx shadcn@latest add <name>` (shadcn MCP 서버 없음)
...
1. **검증**:
   - `mcp__supabase__get_advisors`: 최종 보안/성능 체크
   - `npm run format` → `npm run lint` → `npx tsc --noEmit` → `npm run build`
...
2. **성능 최적화**: Next.js 16.2.12 기능(Streaming, after API, Cache Components 등) 적극 활용
