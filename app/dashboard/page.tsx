...
import { FIXTURE_NOW, sampleDashboard } from '@/lib/fixtures';
import type { EventSummary } from '@/types/database';

/**
 * 대시보드 (F011).
 *
 * Phase 2는 더미 데이터다. `lib/fixtures.ts`를 여기서 직접 호출하고
 * 조회 함수 계층을 두지 않는다 — Task 008에서 이 호출 한 줄만 Supabase
 * 쿼리로 바뀌므로 교체 지점이 명확하다.
 */
...
export default function DashboardPage() {
	const events = sampleDashboard();
	const hosted = splitByTime(events);
	// 참여 목록도 Phase 2에서는 같은 픽스처를 재사용한다.
	const joined = splitByTime(events);
